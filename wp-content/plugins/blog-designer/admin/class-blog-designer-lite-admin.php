...
class Blog_Designer_Lite_Admin {
	...
	public function __construct() {
		...
		add_action( 'admin_notices', array( $this, 'bd_update_notice_hack' ) );
	 ...
	 * @return void
	 */
	public function bd_update_notice_hack() {
		global $wpdb;
		if ( isset( $_GET['blog_designer_hack'] ) && 0 == $_GET['blog_designer_hack'] ) { //phpcs:ignore
		...
		if ( 'yes' !== $blog_designer_hack ) {
			...
			<div class="notice notice-error " id="blog_designer_hack">
				...
				<img style="margin: 20px 20px 10px 10px;float:left" src="<?php echo esc_url( BLOGDESIGNER_URL ); ?>admin/images/blog-designer-vc.png" alt="" />
				<p style="font-size:16px;color:#ff0000"><?php esc_html_e( "Thank you for updating Blog Designer Plugin to latest version. We have added security patches to our plugin. If you found any malicious code other than CSS code in 'Custom CSS' field of", 'blog-designer' ); ?> <a href="<?php echo esc_url( admin_url( 'admin.php?page=designer_settings' ) ); ?>"><?php esc_html_e( 'blog designer settings page', 'blog-designer' ); ?></a>, <?php esc_html_e( 'then please remove that code from that field.', 'blog-designer' ); ?> </p>
			</div>
			<script>
				jQuery(document).ready(function(){
					(function($){
						$('.blog_designer_hack-close').click(function () {
							var data = '';$("#blog_designer_hack").hide();$.post('<?php echo esc_url( admin_url( '?blog_designer_hack=0' ) ); ?>', data, function (response) { });
						});
					}(jQuery))
				});
			</script>
